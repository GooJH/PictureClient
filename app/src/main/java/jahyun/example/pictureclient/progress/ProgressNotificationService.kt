package jahyun.example.pictureclient.progress
...
import jahyun.example.pictureclient.MainActivity
...
class ProgressNotificationService : Service() {
    ...
    private fun setNotification() {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            mNotificationChannel = NotificationChannel(mNotificationChannelId, mNotificationChannelName, NotificationManager.IMPORTANCE_DEFAULT)

            mNotificationChannel.description = "사진 전송이 취소 또는 완료되었을 때 발생하는 알림입니다."
    ...
    private fun setThread() {
        val notificationThread = Thread(Runnable {
            while (!mIsStop) {
                if (mMode == 0) {
